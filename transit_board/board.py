			...
			for directions in ["S","W","N","E"]:
				...
				for keys, values in sorted(arrivals.items()):
					if directions==keys[-1:] and linecount==0:
						text1L=keys
						text1R=','.join(str(t) for t in filterArrivals(values, keys)[:num_arrivals])
						linecount=1
					elif directions==keys[-1:] and linecount==1:
						text2L=keys
						text2R=','.join(str(t) for t in filterArrivals(values, keys)[:num_arrivals])
						linecount=2
					elif directions==keys[-1:] and linecount==2:
						text3L=keys
						text3R=','.join(str(t) for t in filterArrivals(values, keys)[:num_arrivals])
						linecount=3
