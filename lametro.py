...
def fetch_arrivals_swiftly(stops, url, agency, api_key):
	...
	for entity in data.get('entity', []):
		...
		for stu in tu.get('stopTimeUpdate', []):
			...
			arrival_timestamp = int(arrival_timestamp)
			if arrival_timestamp > now + 3600:
				continue
			...
			arrivals[line].append(arrival_timestamp)
...
# 	)
# 	now = time.time()
# 	arrivals_minutes = {line: sorted(round((t - now) / 60) for t in timestamps) for line, timestamps in arrivals.items()}
# 	print("Arrivals:", arrivals_minutes)
